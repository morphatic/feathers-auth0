...
module.exports = app => {
  const usersService = ({
    ...
    async find(params) {
      ...
      if (!this.scopes || !(this.scopes.includes('read:users') || this.scopes.includes('read:user_idp_tokens')))
    ...
    async get(id, params) {
      ...
      return (
        isEmail(id) ?
          this.client.getUsersByEmail(id) :
          this.client.getUser({ id })
      ).then(users => Promise.resolve(users[0] || {}))
    },
    async create(data, params) {
      /**
       * NOTE: This ONLY works for Username-Password-Authentication (i.e. stored within Auth0, not SSO)
       *       A valid passord MUST be passed along with a confirmation
       */
      // throw an error if the token doesn't have the appropriate scopes
      if (!this.scopes || !this.scopes.includes('create:users'))
    ...
    async patch(id, data, params) {
      // throw an error if the token doesn't have the appropriate scopes
      if (!this.scopes || !(this.scopes.includes('update:users') || this.scopes.includes('update:users_app_metadata')))
        throw new Forbidden('The token must have `update:users` or `update:users_app_metadata` scope to call this endpoint')

      // create a list of updateable keys
      const updateable = [
        'connection',
        'client_id',
        'email',
      ...
      Object.keys(data).forEach(
        key => {
          // throw an error if we find a key that is not updateable
    ...
    async remove(id, params) {
      // throw an error if the token doesn't have the appropriate scopes
      if (!this.scopes || !this.scopes.includes('delete:users'))
    ...
    async raw(method, query) {
      // this will run a raw query against apecified method of the Auth0 Management API
    }
  })
